...
const getAsset= async (req, res) => {
    try {
        ...
        const docs = await assetsModel.paginate({}, options);
        res.send(docs);
    } catch (err) {
...
const updateAsset = async (req, res) => {
    ...
    const body = req.body;
    try {
        const docs = await assetsModel.updateOne({ _id: parseId(id) }, body);
        res.send({ items: docs });
    } catch (err) {
...
const createAsset = async (req, res) => {
    try {
        ...
        const newDoc = await assetsModel.create(data);
...
const deleteAsset = async (req, res) => {
    const { id } = req.params;
    try {
        const docs = await assetsModel.deleteOne({ _id: parseId(id) });
        res.send({ items: docs });
    } catch (err) {
...
module.exports = {getAsset, updateAsset, deleteAsset,createAsset};
